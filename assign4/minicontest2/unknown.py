...
Contest Number: 2
Description of Bot: I have two agents (offensive and defensive). The Defensive one is relatively
straightforward, it copys lots of the baseline team code but uses inference to detect where the
attacker/invader is if we have a noisy reading by seeing which dots disappeared on the map in the
previous gameState. The bot can also go on offense when there is no threat, and when it does it uses
all of the offensive tactics that the offensive bot has.

The offensive bot is also a slightly improved version of the baselineTeam.py model.
I add other basic features for the bot such as when to eat the capsule and when to avoid eating
the pacman (if you're scared).
"""
